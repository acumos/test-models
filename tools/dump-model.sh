...
#.
#. Usage:
#. $ bash dump-models.sh <cdsUri> <cdsCreds> <nexusUri> <nexusRepo> <portalUrl>
#                        <active> [solutionId] [version]
...
function dump() {
  ...
  while [[ $i -lt $mets ]] ; do
    ...
    wget --no-check-certificate -O /tmp/acumos/${name}_${solutionId}_${version}/metadata/$met $portalUri/site/binaries/content/assets/solutiondocs/solution/$solutionId/$revisionId/public/$met
    ((i++))
  done
